...
#ifdef DEBUGLOG
        log->LOG(&memberNode->addr, "Message received's size is less than MessageHdr. Ignored.");
#endif
        return false;
    }
...
#ifdef DEBUGLOG
        log->LOG(&memberNode->addr, "Message JOINREP received with wrong size. Ignored. size[%i]", size);
#endif
        return false;
    }
...
bool MP1Node::recvHeartbeatReq(void *env, char *data, int size) {
    if (size < (int)(sizeof(memberNode->addr.addr))) {
#ifdef DEBUGLOG
        log->LOG(&memberNode->addr, "Message HEARTBEATREQ received with wrong size. Ignored.");
#endif
        return false;
    }
...
bool MP1Node::recvHeartbeatRep(void *env, char *data, int size) {
    if (size < (int)(sizeof(memberNode->addr.addr))) {
#ifdef DEBUGLOG
        log->LOG(&memberNode->addr, "Message HEARTBEATREP received with wrong size. Ignored.");
#endif
        return false;
    }
...
#ifdef DEBUGLOG
        log->LOG(&memberNode->addr, "Message %s received with wrong size. Ignored. size[%s]", label, size);
#endif
        return false;
    }
